...
def main():
    ...
    # data_path = '../data'
    data_path = '/home/praveen/dev/slam/warpsight/ORBSLAM_container/GX010040/Results'
    camera_trajectories_path = data_path + '/Trajectories/CameraTrajectories/'
    # Mono-Inertial Trajectories
    
    # /home/praveen/dev/slam/warpsight/ORBSLAM_container/GX010040/Results/Trajectories/CameraTrajectories/MI30FPS.txt
    # /home/praveen/dev/slam/warpsight/ORBSLAM_container/GX010040/Results/trajectories/CameraTrajectories/MI30FPS.txt
    # Monocular Trajectories
    traj_m30 = np.array(read_trajectory_from_file(camera_trajectories_path + 'M30FPS.txt'))
    traj_m60 = np.array(read_trajectory_from_file(camera_trajectories_path + 'M60FPS.txt'))
    traj_m60 = traj_m60[::2][:len(traj_m30)]  # Downsample and clip to match the number of points in sample trajectory
    traj_m120 = np.array(read_trajectory_from_file(camera_trajectories_path + 'M120FPS.txt'))
    traj_m120 = traj_m120[::4][:len(traj_m30)]  # Downsample and clip to match the number of points in sample trajectory
    traj_m240 = np.array(read_trajectory_from_file(camera_trajectories_path + 'M240FPS.txt'))
    traj_m240 = traj_m240[::8][:len(traj_m30)]  # Downsample and clip to match the number of points in sample trajectory

    traj_mi30 = np.array(read_trajectory_from_file(camera_trajectories_path + 'MI30FPS.txt'))
    traj_mi30 = traj_mi30[:len(traj_m30)]  # Downsample and clip to match the number of points in sample trajectory
    traj_mi60 = np.array(read_trajectory_from_file(camera_trajectories_path + 'MI60FPS.txt'))
    traj_mi60 = traj_mi60[::2][:len(traj_m30)]  # Downsample and clip to match the number of points in sample trajectory

    # Transform monocular trajectories to line up with reference
    print(traj_mi30.shape, traj_m30.shape)
    ...
    traj_mi_imu = np.array(read_trajectory_from_file(camera_trajectories_path + 'imu.txt'))
    traj_mi_optical = np.array(read_trajectory_from_file(camera_trajectories_path + 'optical_flow.txt'))
